...
jQuery(document).ready(function($) {
    console.log('🔍 求人検索スクリプトを読み込みました');
    
    // グローバル変数の安全な取得
    var ajaxurl = (typeof job_search_params !== 'undefined') ? job_search_params.ajax_url : '/wp-admin/admin-ajax.php';
    var site_url = (typeof job_search_params !== 'undefined') ? job_search_params.site_url : '';
    var search_nonce = (typeof job_search_params !== 'undefined') ? job_search_params.nonce : '';
    ...
    // 詳細検索の表示/非表示切り替え
    $('#detail-toggle-btn').on('click', function(e) {
        e.preventDefault();
    ...
    // エリアフィールドクリック
    $(document).on('click', '#area-field', function(e) {
        e.preventDefault();
        e.stopPropagation();
        console.log('🎯 エリアフィールドがクリックされました');
        openModal('area-modal-overlay');
        showAreaStep1();
    });
    
    // 職種フィールドクリック
    $(document).on('click', '#position-field', function(e) {
        e.preventDefault();
        e.stopPropagation();
        console.log('🎯 職種フィールドがクリックされました');
        openModal('position-modal-overlay');
    });
    
    // 雇用形態フィールドクリック
    $(document).on('click', '#job-type-field', function(e) {
        e.preventDefault();
        e.stopPropagation();
        console.log('🎯 雇用形態フィールドがクリックされました');
        openModal('job-type-modal-overlay');
    });
    
    // 施設形態フィールドクリック
    $(document).on('click', '#facility-type-field', function(e) {
        e.preventDefault();
        e.stopPropagation();
        console.log('🎯 施設形態フィールドがクリックされました');
        openModal('facility-type-modal-overlay');
    });
    
    // 特徴フィールドクリック
    $(document).on('click', '#feature-field', function(e) {
        e.preventDefault();
        e.stopPropagation();
        console.log('🎯 特徴フィールドがクリックされました');
    ...
    // トップレベルのエリア選択
    $(document).on('click', '.area-btn', function(e) {
        e.preventDefault();
        console.log('🌍 エリアボタンがクリックされました');
    ...
    // 「全域で検索」ボタン（第1階層）
    $(document).on('click', '#select-area-btn', function(e) {
        e.preventDefault();
        console.log('🌍 全域で検索（第1階層）');
    ...
    // 第2階層のターム選択
    $(document).on('click', '.prefecture-btn', function(e) {
        e.preventDefault();
        console.log('🏙️ 都道府県ボタンがクリックされました');
    ...
    // 「全域で検索」ボタン（第2階層）
    $(document).on('click', '#select-prefecture-btn', function(e) {
        e.preventDefault();
        console.log('🏙️ 全域で検索（第2階層）');
    ...
    $(document).on('click', '.city-btn', function(e) {
        e.preventDefault();
        console.log('🏘️ 市区町村ボタンがクリックされました');
    ...
    // 職種選択
    $(document).on('click', '.position-btn', function(e) {
        e.preventDefault();
        console.log('💼 職種ボタンがクリックされました');
    ...
    // 雇用形態選択
    $(document).on('click', '.job-type-btn', function(e) {
        e.preventDefault();
        console.log('🏢 雇用形態ボタンがクリックされました');
    ...
    // 施設形態選択
    $(document).on('click', '.facility-type-btn', function(e) {
        e.preventDefault();
        console.log('🏥 施設形態ボタンがクリックされました');
    ...
    // 特徴の適用ボタン
    $(document).on('click', '#apply-features-btn', function(e) {
        e.preventDefault();
        console.log('✨ 特徴適用ボタンがクリックされました');
    ...
    $(document).on('click', '#search-btn', function(e) {
        ...
        if (!hasPathFilters && keyword) {
            // キーワードのみ検索
            targetUrl = '/?s=' + encodeURIComponent(keyword) + '&post_type=job';
    ...
    function loadSecondLevelTerms(parentId) {
        console.log('📡 第2階層タームを読み込み中...', parentId);
        ...
        $.ajax({
            ...
            data: {
                ...
                _wpnonce: search_nonce
            },
            beforeSend: function() {
                $('#prefecture-grid').html('<p>読み込み中...</p>');
            },
            success: function(response) {
                console.log('📡 第2階層レスポンス:', response);
                if (response.success) {
                    displaySecondLevelTerms(response.data);
                } else {
                    $('#prefecture-grid').html('<p>都道府県が見つかりませんでした</p>');
                }
            },
    ...
    function loadThirdLevelTerms(parentId) {
        console.log('📡 第3階層タームを読み込み中...', parentId);
        ...
        $.ajax({
            ...
            beforeSend: function() {
                $('#city-grid').html('<p>読み込み中...</p>');
            },
            success: function(response) {
                console.log('📡 第3階層レスポンス:', response);
                if (response.success) {
    ...
    function updateSelectionDisplay(fieldSelector, text) {
        ...
        var $display = $field.find('.selection-display');
        if ($display.length > 0) {
            $display.text(text);
            $display.removeClass('selection-placeholder');
        }
    }
