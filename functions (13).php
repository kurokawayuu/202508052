...
// 既存の処理をすべて削除
remove_action('admin_post_delete_my_account', 'handle_delete_account');
remove_action('admin_post_delete_my_account', 'improved_handle_delete_account');
remove_action('admin_post_nopriv_delete_my_account', 'improved_handle_delete_account');

/**
 * 直接処理版：退会処理
 */
function direct_withdrawal_handler() {
    // members ページでPOSTリクエストがあった場合のみ処理
    if (!is_page_template('page-members.php') && !is_page('members')) {
        return;
    }
    ...
    if (!isset($_POST['direct_withdrawal_action'])) {
        return;
    }
    ...
    // nonce確認
    if (!isset($_POST['withdrawal_nonce']) || !wp_verify_nonce($_POST['withdrawal_nonce'], 'direct_withdrawal_nonce')) {
        wp_die('セキュリティチェックに失敗しました。');
    }
    
    // 確認チェックボックス
    if (!isset($_POST['confirm_deletion'])) {
        wp_redirect(add_query_arg(array('_tab' => 'withdrawal', 'error' => 'no_confirmation'), home_url('/members/')));
        exit;
    }
    ...
    // ログに記録
    error_log("=== 直接退会処理開始 ===");
    error_log("ユーザーID: {$user_id}");
    error_log("メールアドレス: {$user_email}");
    error_log("表示名: {$user_name}");
    
    // メール送信
    $email_sent = direct_send_withdrawal_email($user_email, $user_name);
    error_log("退会メール送信結果: " . ($email_sent ? 'SUCCESS' : 'FAILED'));
    ...
    error_log("ユーザー削除結果: " . ($deletion_result ? 'SUCCESS' : 'FAILED'));
    error_log("=== 直接退会処理完了 ===");
    
    // リダイレクト
    wp_redirect(home_url('/?withdrawal_completed=true'));
    exit;
}
add_action('template_redirect', 'direct_withdrawal_handler', 1);

/**
 * 退会完了メール送信
 */
function direct_send_withdrawal_email($user_email, $user_name) {
    $site_name = get_bloginfo('name');
    $subject = "[{$site_name}] 退会完了のお知らせ";
    
    $message = "{$user_name} 様\n\n";
    $message .= "退会手続きが完了しました。\n";
    $message .= "ご利用いただき、ありがとうございました。\n\n";
    $message .= "-----\n";
    $message .= $site_name . "\n";
    $message .= home_url() . "\n";
 ...
 * 退会完了メッセージ表示
 */
function show_withdrawal_completed_message() {
    if (isset($_GET['withdrawal_completed']) && $_GET['withdrawal_completed'] === 'true') {
        ?>
        <div style="
            position: fixed;
            top: 0;
        ...
        ">
            ...
            ">
                <h2 style="color: #28a745; margin-bottom: 20px;">✅ 退会完了</h2>
...
add_action('wp_head', 'show_withdrawal_completed_message');

/**
 * 緊急デバッグ関数
 */
function emergency_debug_withdrawal() {
    if (isset($_GET['debug_emergency']) && current_user_can('administrator')) {
        echo '<div style="background: red; color: white; padding: 20px; position: fixed; top: 0; left: 0; z-index: 99999; width: 100%;">';
        echo '<h3>🚨 緊急デバッグ情報</h3>';
        
        echo '<p><strong>現在のページ:</strong> ' . get_the_title() . '</p>';
        echo '<p><strong>テンプレート:</strong> ' . get_page_template_slug() . '</p>';
        echo '<p><strong>リクエストメソッド:</strong> ' . $_SERVER['REQUEST_METHOD'] . '</p>';
        
        if (is_user_logged_in()) {
            $user = wp_get_current_user();
            echo '<p><strong>ログイン中:</strong> ' . $user->display_name . ' (ID: ' . $user->ID . ')</p>';
        } else {
            echo '<p><strong>ログイン状態:</strong> 未ログイン</p>';
        }
        
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            echo '<p><strong>POST データ:</strong></p>';
            echo '<pre style="background: rgba(255,255,255,0.2); padding: 10px; font-size: 12px; max-height: 200px; overflow-y: scroll;">';
            print_r($_POST);
            echo '</pre>';
        }
        ...
        if (file_exists($log_file)) {
            ...
            echo '<p><strong>最新ログ:</strong></p>';
            echo '<pre style="background: rgba(255,255,255,0.2); padding: 10px; font-size: 11px; max-height: 150px; overflow-y: scroll;">';
...
add_action('wp_head', 'emergency_debug_withdrawal');
 ...
 * 簡易テスト関数（管理者のみ）
 */
function test_withdrawal_system() {
    if (isset($_GET['test_system']) && current_user_can('administrator')) {
        echo '<div style="background: yellow; padding: 20px; margin: 20px;">';
        echo '<h3>🧪 退会システムテスト</h3>';
        
        // メール送信テスト
        $test_email = direct_send_withdrawal_email(get_option('admin_email'), 'テストユーザー');
        echo '<p>メール送信テスト: ' . ($test_email ? '✅ 成功' : '❌ 失敗') . '</p>';
        
        // wp_delete_user関数の存在確認
        echo '<p>wp_delete_user関数: ' . (function_exists('wp_delete_user') ? '✅ 利用可能' : '❌ 利用不可') . '</p>';
        
        // ログファイルの書き込み権限
        $log_file = WP_CONTENT_DIR . '/debug.log';
        echo '<p>ログファイル書き込み: ' . (is_writable(dirname($log_file)) ? '✅ 可能' : '❌ 不可') . '</p>';
        
        echo '</div>';
    }
}
add_action('wp_footer', 'test_withdrawal_system');
...
function can_user_edit_job($job_id, $user_id = null) {
    ...
    // agencyロールを持つユーザーで、基本的な編集権限があれば可能
    $user = get_userdata($user_id);
    if ($user && in_array('agency', (array)$user->roles)) {
        // 基本的な投稿編集権限をチェック
        if (user_can($user_id, 'edit_jobs') || user_can($user_id, 'edit_posts')) {
            return true;
        }
    }
...
function ensure_agency_job_permissions() {
    ...
    if ($role) {
        ...
        $role->add_cap('edit_published_jobs', true);
        $role->add_cap('delete_published_jobs', true);
        $role->add_cap('edit_private_jobs', true);
        $role->add_cap('delete_private_jobs', true);
        
        // メディア関連権限
        $role->add_cap('upload_files', true);
        
        // 他のユーザーの投稿は編集不可（安全のため）
        $role->add_cap('edit_others_jobs', false);
        $role->add_cap('delete_others_jobs', false);
        $role->add_cap('read_private_jobs', false);
    }
}
add_action('init', 'ensure_agency_job_permissions', 5);
...
/**
 * 求人ページネーション根本修正版
 * 404エラーを完全に解決するための最終コード
 */

/**
 * 既存のすべてのリライトルール関数を削除
 */
remove_action('init', 'job_search_rewrite_rules');
remove_action('init', 'add_custom_job_rewrite_rules');
remove_action('init', 'enhanced_job_search_rewrite_rules');
remove_action('init', 'fixed_job_search_rewrite_rules');
remove_action('init', 'ultimate_job_rewrite_rules');

/**
 * 完全に新しいリライトルールシステム
 */
function final_job_rewrite_system() {
    // 高優先度でルールを追加
    ...
    // ログに記録
    error_log('Final job rewrite rules added with high priority');
}
add_action('init', 'final_job_rewrite_system', 1);

/**
 * クエリ変数の確実な登録
 */
function final_job_query_vars($vars) {
    if (!in_array('job_position', $vars)) {
        $vars[] = 'job_position';
    }
    if (!in_array('job_location', $vars)) {
        $vars[] = 'job_location';
    }
    if (!in_array('job_type', $vars)) {
        $vars[] = 'job_type';
    }
    if (!in_array('facility_type', $vars)) {
        $vars[] = 'facility_type';
    }
    if (!in_array('job_feature', $vars)) {
        $vars[] = 'job_feature';
    }
    return $vars;
}
add_filter('query_vars', 'final_job_query_vars', 1);

/**
 * 404を防ぐためのクエリ処理
 */
function final_job_query_handling($query) {
    // 管理画面やAJAXでは処理しない
    if (is_admin() || defined('DOING_AJAX')) {
        return;
    }
    ...
    // job_position クエリ変数がある場合
    $job_position = get_query_var('job_position');
    if (!empty($job_position)) {
        ...
        error_log("Job position query handled: position={$job_position}, paged={$paged}");
    }
    
    // job_location クエリ変数がある場合も同様に処理
    $job_location = get_query_var('job_location');
...
add_action('pre_get_posts', 'final_job_query_handling', 1);

/**
 * テンプレートリダイレクトで404を完全に防ぐ
 */
function final_job_template_redirect() {
    global $wp_query;
    
    // job_position または job_location クエリがある場合
    if (get_query_var('job_position') || get_query_var('job_location')) {
        // 404状態を強制的に解除
        ...
        error_log('Template redirect: 404 status cleared for job query');
    }
}
add_action('template_redirect', 'final_job_template_redirect', 1);

/**
 * 強制的なリライトルールフラッシュ
 */
function force_final_flush() {
    $option_key = 'final_job_rules_v5';
    ...
    if (!get_option($option_key)) {
        ...
        error_log('Final job rewrite rules force flushed - v5');
    }
}
add_action('init', 'force_final_flush', 999);

/**
 * 直接的な404処理を無効化
 */
function prevent_job_404($template) {
    global $wp_query;
    
    // job関連のクエリ変数がある場合は404テンプレートを使用しない
    if (get_query_var('job_position') || get_query_var('job_location')) {
        if (is_404()) {
            $wp_query->is_404 = false;
            status_header(200);
...
add_filter('template_include', 'prevent_job_404', 99);

/**
 * ページネーション用URL生成
 */
function get_final_job_pagination() {
    global $wp_query;
 ...
 * テンプレート用表示関数
 */
function display_final_job_pagination() {
    echo '<div class="pagination">' . get_final_job_pagination() . '</div>';
}

/**
 * 詳細なデバッグ情報
 */
function final_debug_info() {
    ...
    ?>
    <div style="background: #f9f9f9; border: 2px solid #333; padding: 20px; margin: 20px; font-family: monospace; font-size: 12px; z-index: 9999; position: relative;">
        <h2 style="color: red;">🔍 求人ページネーション デバッグ情報</h2>
        
        <h3>📍 基本情報</h3>
        <p><strong>現在のURL:</strong> <?php echo $_SERVER['REQUEST_URI']; ?></p>
        <p><strong>HTTP Status:</strong> <?php echo http_response_code(); ?></p>
        <p><strong>is_404:</strong> <?php echo is_404() ? '❌ YES (404エラー)' : '✅ NO'; ?></p>
        <p><strong>is_archive:</strong> <?php echo is_archive() ? '✅ YES' : '❌ NO'; ?></p>
        <p><strong>post_type:</strong> <?php echo get_query_var('post_type') ?: 'なし'; ?></p>
        
        <h3>🎯 クエリ変数</h3>
        <p><strong>job_position:</strong> <?php echo get_query_var('job_position') ?: 'なし'; ?></p>
        <p><strong>job_location:</strong> <?php echo get_query_var('job_location') ?: 'なし'; ?></p>
        <p><strong>paged:</strong> <?php echo get_query_var('paged') ?: 'なし'; ?></p>
        
        <h3>📊 クエリ結果</h3>
        <p><strong>found_posts:</strong> <?php echo $wp_query->found_posts; ?></p>
        <p><strong>max_num_pages:</strong> <?php echo $wp_query->max_num_pages; ?></p>
        <p><strong>current_post:</strong> <?php echo $wp_query->current_post; ?></p>
        
        <h3>🔧 リライトルール（jobs関連）</h3>
        <pre style="background: #fff; padding: 10px; max-height: 300px; overflow-y: scroll;">
<?php
        foreach ($wp_rewrite->rules as $pattern => $replacement) {
            if (strpos($pattern, 'jobs') !== false) {
                echo htmlspecialchars($pattern) . ' => ' . htmlspecialchars($replacement) . "\n";
            }
        }
?>
        </pre>
        ...
        <h3>🎛️ 全クエリ変数</h3>
        <pre style="background: #fff; padding: 10px; max-height: 200px; overflow-y: scroll;">
<?php print_r($wp_query->query_vars); ?>
        </pre>
        
        <h3>🛠️ アクション</h3>
        <p>
            <a href="<?php echo admin_url('edit.php?post_type=job&force_flush_final=1'); ?>" style="background: #0073aa; color: white; padding: 10px; text-decoration: none; border-radius: 3px;">
                🔄 リライトルールを強制フラッシュ
            </a>
        </p>
    </div>
...
add_action('wp_footer', 'final_debug_info');

/**
 * 管理画面から強制フラッシュ
 */
function admin_force_flush_final() {
    if (current_user_can('administrator') && isset($_GET['force_flush_final'])) {
        delete_option('final_job_rules_v5');
        global $wp_rewrite;
        $wp_rewrite->flush_rules(true);
        
        wp_redirect(admin_url('edit.php?post_type=job&flushed_final=1'));
        exit;
    }
    
    if (isset($_GET['flushed_final'])) {
        echo '<div class="notice notice-success"><p>✅ リライトルールを強制フラッシュしました。ページネーションをテストしてください。</p></div>';
    }
}
add_action('admin_init', 'admin_force_flush_final');

/**
 * 不足しているJavaScriptファイルエラーを修正
 */
function fix_missing_js_css_files() {
    // 存在しないファイルの読み込みを防ぐ
    wp_dequeue_script('job-search');
    wp_dequeue_script('favorite-job-script');
    wp_dequeue_style('job-search-style');
    
    // 代替として既存のスクリプトを確認して読み込み
    $theme_dir = get_stylesheet_directory();
    $theme_uri = get_stylesheet_directory_uri();
    
    // 実際に存在するファイルのみ読み込み
    if (file_exists($theme_dir . '/js/job-search.js')) {
        wp_enqueue_script('job-search-fixed', $theme_uri . '/js/job-search.js', array('jquery'), '1.0.1', true);
    }
    
    if (file_exists($theme_dir . '/js/favorite-job.js')) {
        wp_enqueue_script('favorite-job-fixed', $theme_uri . '/js/favorite-job.js', array('jquery'), '1.0.1', true);
    }
    
    if (file_exists($theme_dir . '/css/job-search.css')) {
        wp_enqueue_style('job-search-fixed', $theme_uri . '/css/job-search.css', array(), '1.0.1');
    }
}
add_action('wp_enqueue_scripts', 'fix_missing_js_css_files', 999);

/**
 * Mixed Contentエラーを修正
 */
function fix_mixed_content_images($content) {
    // httpをhttpsに変換
    $content = str_replace('http://testjc-fc.kphd-portal.net/', 'https://testjc-fc.kphd-portal.net/', $content);
    return $content;
}
add_filter('the_content', 'fix_mixed_content_images');
add_filter('wp_get_attachment_url', 'fix_mixed_content_images');

/**
 * 緊急デバッグ：直接URLパターンをチェック
 */
function emergency_url_debug() {
    if (isset($_GET['emergency_debug'])) {
        ...
        // パターンマッチングをテスト
        if (preg_match('#^/jobs/position/([^/]+)/page/([0-9]+)/?#', $request_uri, $matches)) {
            echo '<p>✅ URL パターンマッチ成功</p>';
            echo '<p>Position: ' . $matches[1] . '</p>';
            echo '<p>Page: ' . $matches[2] . '</p>';
        } else {
            echo '<p>❌ URL パターンマッチ失敗</p>';
        }
        
        echo '</div>';
    }
}
add_action('wp_head', 'emergency_url_debug');
...
/**
 * 求人URL完全対応版
 * /jobs/chubu/pt/1318/ 形式の404エラーを完全解決
 * functions.phpに追加してください
 */

// === 既存のすべてのリライトルール関数を無効化 ===
remove_action('init', 'job_search_rewrite_rules');
remove_action('init', 'add_custom_job_rewrite_rules');
remove_action('init', 'enhanced_job_search_rewrite_rules');
remove_action('init', 'fixed_job_search_rewrite_rules');
remove_action('init', 'ultimate_job_rewrite_rules');
remove_action('init', 'final_job_rewrite_system');
remove_action('init', 'ultimate_fixed_job_rewrite_rules');

/**
 * 完全URL対応リライトルールシステム
 */
function complete_job_rewrite_system() {
    error_log('=== Complete Job Rewrite System Started ===');
    
    // === 1. 年アーカイブルール削除 ===
    global $wp_rewrite;
    $year_rules_to_remove = array(
        '([0-9]{4})/?$',
        '([0-9]{4})/page/?([0-9]{1,})/?$',
    ...
    foreach ($year_rules_to_remove as $rule) {
        if (isset($wp_rewrite->rules[$rule])) {
    ...
    // パターン1: /jobs/地域/職種/ID/ (例: /jobs/chubu/pt/1318/)
    add_rewrite_rule(
        '^jobs/([^/]+)/([^/]+)/([0-9]+)/?$',
        'index.php?post_type=job&p=$matches[3]&job_location_slug=$matches[1]&job_position_slug=$matches[2]',
        'top'
    );
    
    // パターン2: /jobs/地域/職種略称/ID/ (職種略称の特別対応)
    add_rewrite_rule(
        '^jobs/([^/]+)/(pt|ft|contract|part|full)/([0-9]+)/?$',
        'index.php?post_type=job&p=$matches[3]&job_location_slug=$matches[1]&job_type_slug=$matches[2]',
        'top'
    );
    
    // パターン3: /jobs/数字のみ/ (シンプル形式)
    add_rewrite_rule(
        '^jobs/([0-9]+)/?$',
        'index.php?post_type=job&p=$matches[1]',
        'top'
    ...
    error_log('Complete job rewrite rules added successfully');
}
add_action('init', 'complete_job_rewrite_system', 1);

/**
 * カスタムクエリ変数の登録
 */
function complete_job_query_vars($vars) {
    $custom_vars = array(
        'job_location_slug',
        'job_position_slug', 
        'job_type_slug',
    ...
    error_log('Complete job query vars registered: ' . implode(', ', $custom_vars));
    return $vars;
}
add_filter('query_vars', 'complete_job_query_vars', 1);

/**
 * 完全なクエリ処理システム
 */
function complete_job_query_processing($query) {
    // 管理画面、AJAX、サブクエリでは処理しない
    if (is_admin() || defined('DOING_AJAX') || !$query->is_main_query()) {
        return;
    }
    
    $request_uri = $_SERVER['REQUEST_URI'];
    error_log("Processing URL: {$request_uri}");
    
    // === パターン1: /jobs/地域/職種/ID/ ===
    if (preg_match('#^/jobs/([^/]+)/([^/]+)/([0-9]+)/?$#', $request_uri, $matches)) {
        $location_slug = $matches[1];
        $position_or_type_slug = $matches[2];
        $job_id = intval($matches[3]);
        
        error_log("Pattern 1 matched: location={$location_slug}, position_or_type={$position_or_type_slug}, job_id={$job_id}");
        ...
        if ($job && $job->post_type === 'job') {
            ...
            $query->set('post_type', 'job');
            $query->set('p', $job_id);
            $query->set('job_location_slug', $location_slug);
            
            // 職種略称の判定
            $job_type_abbreviations = array('pt', 'ft', 'contract', 'part', 'full');
            if (in_array($position_or_type_slug, $job_type_abbreviations)) {
                $query->set('job_type_slug', $position_or_type_slug);
            } else {
                $query->set('job_position_slug', $position_or_type_slug);
            }
            ...
            error_log("Job detail query set successfully for ID: {$job_id}");
            return;
        } else {
            error_log("Job ID {$job_id} not found or not job post type");
        }
    }
    ...
    if (preg_match('#^/jobs/([0-9]+)/?$#', $request_uri, $matches)) {
        ...
        error_log("Pattern 2 matched: job_id={$job_id}");
        ...
        if ($job && $job->post_type === 'job') {
            ...
            error_log("Simple job detail query set for ID: {$job_id}");
            return;
        }
    }
    ...
    if (!empty($job_position) || !empty($job_location)) {
        $query->set('post_type', 'job');
        $query->is_404 = false;
        $query->is_home = false;
        $query->is_archive = true;
        ...
        error_log("Archive query processed: position={$job_position}, location={$job_location}");
    }
}
add_action('pre_get_posts', 'complete_job_query_processing', 1);

/**
 * テンプレートリダイレクトでの最終404防止
 */
function complete_job_template_redirect() {
    global $wp_query;
    
    if (!is_404()) {
        return; // 404でなければ何もしない
    }
    
    $request_uri = $_SERVER['REQUEST_URI'];
    error_log("404 detected for: {$request_uri}");
    
    // 求人URLパターンで404の場合
    if (strpos($request_uri, '/jobs/') === 0) {
        // パターン1: /jobs/地域/職種/ID/
        if (preg_match('#^/jobs/([^/]+)/([^/]+)/([0-9]+)/?$#', $request_uri, $matches)) {
            $job_id = intval($matches[3]);
            $job = get_post($job_id);
            ...
            if ($job && $job->post_type === 'job') {
                error_log("Template redirect: Fixing 404 for job ID {$job_id}");
                ...
                // クエリオブジェクトを設定
                $wp_query->queried_object = $job;
                $wp_query->queried_object_id = $job_id;
                $wp_query->post = $job;
                $wp_query->posts = array($job);
        ...
        if (preg_match('#^/jobs/([0-9]+)/?$#', $request_uri, $matches)) {
            ...
            if ($job && $job->post_type === 'job') {
                error_log("Template redirect: Simple pattern fix for job ID {$job_id}");
        ...
        // 最後の手段：求人一覧にリダイレクト
        error_log("Final fallback: Redirecting to jobs archive");
        wp_redirect(home_url('/jobs/'), 301);
        exit;
    }
}
add_action('template_redirect', 'complete_job_template_redirect', 1);

/**
 * 年アーカイブルールの完全削除
 */
function remove_year_archive_rules($rules) {
    $year_patterns = array(
        '([0-9]{4})/?$',
        '([0-9]{4})/page/?([0-9]{1,})/?$',
        '([0-9]{4})/feed/(feed|rdf|rss|rss2|atom)/?$',
    ...
    foreach ($year_patterns as $pattern) {
        if (isset($rules[$pattern])) {
            unset($rules[$pattern]);
            error_log("Removed year archive rule: {$pattern}");
        }
    }
    
    return $rules;
}
add_filter('rewrite_rules_array', 'remove_year_archive_rules', 1);

/**
 * 強制リライトルールフラッシュ
 */
function force_complete_flush() {
    $option_key = 'complete_job_rules_v2';
    
    if (!get_option($option_key)) {
        error_log('Forcing complete rewrite rules flush...');
        ...
        error_log('Complete job rewrite rules flushed successfully');
    }
}
add_action('init', 'force_complete_flush', 999);

/**
 * デバッグ機能（管理者用）
 */
function debug_complete_job_system() {
    if (!current_user_can('administrator') || !isset($_GET['debug_complete'])) {
        return;
    }
    ...
    echo '<div style="background: #fff; border: 3px solid #e74c3c; padding: 20px; margin: 20px; font-family: Consolas, monospace; font-size: 11px; position: fixed; top: 0; left: 0; z-index: 99999; max-width: 90vw; max-height: 90vh; overflow: auto;">';
    echo '<h2 style="color: #e74c3c; margin: 0 0 15px 0;">🔧 完全デバッグ情報</h2>';
    
    echo '<h3>📍 現在の状況</h3>';
    echo '<p><strong>URL:</strong> ' . $request_uri . '</p>';
    echo '<p><strong>HTTP Status:</strong> ' . http_response_code() . '</p>';
    echo '<p><strong>is_404:</strong> ' . (is_404() ? '❌ YES (404エラー)' : '✅ NO') . '</p>';
    echo '<p><strong>is_single:</strong> ' . (is_single() ? '✅ YES' : '❌ NO') . '</p>';
    echo '<p><strong>post_type:</strong> ' . (get_query_var('post_type') ?: '未設定') . '</p>';
    
    echo '<h3>🎯 URLパターン解析</h3>';
    
    // パターン1のテスト
    if (preg_match('#^/jobs/([^/]+)/([^/]+)/([0-9]+)/?$#', $request_uri, $matches)) {
        echo '<p>✅ パターン1マッチ (地域/職種/ID)</p>';
        echo '<ul>';
        echo '<li>地域: ' . $matches[1] . '</li>';
        echo '<li>職種/タイプ: ' . $matches[2] . '</li>';
        echo '<li>求人ID: ' . $matches[3] . '</li>';
        echo '</ul>';
        
        $job = get_post(intval($matches[3]));
        echo '<p>求人存在: ' . ($job && $job->post_type === 'job' ? '✅ YES' : '❌ NO') . '</p>';
    } elseif (preg_match('#^/jobs/([0-9]+)/?$#', $request_uri, $matches)) {
        echo '<p>✅ パターン2マッチ (数字のみ)</p>';
        echo '<p>求人ID: ' . $matches[1] . '</p>';
    } else {
        echo '<p>❌ どのパターンにもマッチしません</p>';
    }
    
    echo '<h3>📊 クエリ情報</h3>';
    echo '<ul>';
    echo '<li><strong>found_posts:</strong> ' . ($wp_query->found_posts ?? '未設定') . '</li>';
    echo '<li><strong>post_count:</strong> ' . ($wp_query->post_count ?? '未設定') . '</li>';
    echo '<li><strong>queried_object_id:</strong> ' . ($wp_query->queried_object_id ?? '未設定') . '</li>';
    echo '</ul>';
    
    if ($wp_query->post) {
        echo '<p><strong>現在の投稿:</strong> ' . $wp_query->post->post_title . ' (ID: ' . $wp_query->post->ID . ', Type: ' . $wp_query->post->post_type . ')</p>';
    }
    
    echo '<h3>🔧 操作</h3>';
    echo '<p><a href="' . admin_url('edit.php?post_type=job&force_complete_flush=1') . '" style="background: #e74c3c; color: white; padding: 8px 12px; text-decoration: none; border-radius: 3px;">🔄 リライトルール強制フラッシュ</a></p>';
    
    echo '</div>';
}
add_action('wp_footer', 'debug_complete_job_system');

/**
 * 管理画面からの強制フラッシュ
 */
function admin_complete_flush() {
    if (current_user_can('administrator') && isset($_GET['force_complete_flush'])) {
        delete_option('complete_job_rules_v2');
        global $wp_rewrite;
        $wp_rewrite->flush_rules(true);
        
        wp_redirect(admin_url('edit.php?post_type=job&complete_flushed=1'));
        exit;
    }
    
    if (isset($_GET['complete_flushed'])) {
        add_action('admin_notices', function() {
            echo '<div class="notice notice-success"><p>✅ 完全リライトルールをフラッシュしました！テストしてください。</p></div>';
        });
    }
}
add_action('admin_init', 'admin_complete_flush');

/**
 * .htaccess緊急バックアップ（最後の手段）
 */
function emergency_htaccess_rules() {
    if (current_user_can('administrator') && isset($_GET['show_htaccess'])) {
        echo '<div style="background: #2c3e50; color: #ecf0f1; padding: 20px; margin: 20px; font-family: Consolas, monospace;">';
        echo '<h3>緊急用 .htaccess ルール</h3>';
        echo '<p>functions.phpでの修正がうまくいかない場合、以下を.htaccessに追加：</p>';
        echo '<pre style="background: #34495e; padding: 15px; border-radius: 5px;">';
        echo '# 求人詳細ページ緊急ルール' . "\n";
        echo 'RewriteRule ^jobs/([^/]+)/([^/]+)/([0-9]+)/?$ /index.php?post_type=job&p=$3 [QSA,L]' . "\n";
        echo 'RewriteRule ^jobs/([0-9]+)/?$ /index.php?post_type=job&p=$1 [QSA,L]' . "\n";
        echo "\n";
        echo '# 年アーカイブ無効化' . "\n";
        echo 'RewriteRule ^([0-9]{4})/?$ - [R=404,L]';
        echo '</pre>';
        echo '</div>';
    }
}
add_action('wp_footer', 'emergency_htaccess_rules');